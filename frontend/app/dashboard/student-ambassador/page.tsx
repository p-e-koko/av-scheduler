...
function StudentAmbassadorDashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!hasAnyRole(['student_ambassador', 'admin'])) {
        router.push('/login')
        return
      }
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6">
                                ...
                                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
                                  {assignment.pivot.status !== 'accepted' && assignment.pivot.status !== 'rejected' && (
                                    <>
                                      ...
                                      <Button
                                        size="sm"
                                        className="flex-1 sm:flex-none bg-red-900 hover:bg-red-950 text-white shadow-sm hidden"
                                        onClick={() => handleRejectAssignment(assignment.id)}
                                        disabled
                                      >
                                        Reject
          ...
          {activeTab === "supervisor-schedule" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-foreground">Supervisor Schedule</h2>
              </div>
              <MarketingSupervisorSimpleSchedule />
            </div>
          )}
...
export default function ProtectedStudentAmbassadorDashboard() {
  return (
