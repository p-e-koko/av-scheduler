    ...
    <div className="bg-card/80 backdrop-blur-xl border-r border-border shadow-lg shadow-gray-100/50 dark:shadow-none h-full flex flex-col">
          ...
          {!sidebarCollapsed || isMobile ? (
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-primary-foreground/80">Coordinator Dashboard</p>
              </div>
            </div>
          ) : (
            <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center">
      ...
      <div className="flex-1 p-2">
        <nav className="space-y-1">
          ...
          {currentUser && getAllowedDashboards(currentUser.roles || []).filter(path => !path.includes('/dashboard/coordinator') && !path.includes('/dashboard/inventory') && !path.includes('/dashboard/keys')).length > 0 && (
            <>
              ...
              {getAllowedDashboards(currentUser.roles || [])
                .filter(path => !path.includes('/dashboard/coordinator') && !path.includes('/dashboard/inventory') && !path.includes('/dashboard/keys'))
                .map(path => {
