...
interface MarketingSupervisorSidebarProps {
    ...
    onTabChange: (tab: MarketingSupTab) => void
    isOpen?: boolean
    onClose?: () => void
    user?: UserType | null
}

export function MarketingSupervisorSidebar({
    activeTab,
    onTabChange,
    isOpen,
    onClose,
    user,
}: MarketingSupervisorSidebarProps) {
                ...
                <div className={`flex items-center ${sidebarCollapsed && !isMobile ? 'justify-center' : 'space-x-3'}`}>
                    ...
                    {(!sidebarCollapsed || isMobile) && (
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground truncate">Marketing Supervisor</p>
                        </div>
                    )}
                    {(!sidebarCollapsed || isMobile) && (
